...
interface ShortlistJob {
  ...
  source: string; remote: 0|1; salary: string; date_posted: string
  ...
  status: string; notes: string; date_added: string; date_updated: string
}
...
const { data, pending, refresh } = useFetch('/api/shortlist', {
  query: computed(() => statusFilter.value ? { status: statusFilter.value } : {}),
})
const jobs = computed(() => (data.value ?? []) as ShortlistJob[])

const statusCounts = computed(() => {
  const counts: Record<string, number> = {}
  for (const j of (data.value ?? [])) counts[j.status] = (counts[j.status] ?? 0) + 1
  return counts
})
...
async function updateStatus(job: ShortlistJob, status: string) {
  await $fetch(`/api/jobs/${job.id}/shortlist`, {
    method: 'POST', body: { status, notes: job.notes }
  })
  await refresh()
}
...
<template>
  <UContainer class="py-6 space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h1 class="text-2xl font-bold">My Shortlist</h1>
        <p class="text-muted text-sm mt-1">{{ jobs.length }} saved roles</p>
      </div>
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <UBadge
        v-for="[status, count] in Object.entries(statusCounts)" :key="status"
    ...
    <div v-if="pending" class="text-center text-muted py-12">Loading…</div>
    <div v-else-if="!jobs.length" class="text-center text-muted py-12">
      <UIcon name="i-lucide-star" class="text-4xl mb-3" />
      <p>No jobs shortlisted yet. Go to the Jobs board and star roles you're interested in.</p>
      <UButton to="/" class="mt-4">Browse Jobs</UButton>
    </div>
      ...
      >
        <div class="flex items-start justify-between gap-3">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <UBadge v-if="job.score" :color="job.score >= 8 ? 'success' : job.score >= 6.5 ? 'primary' : 'warning'" class="font-mono font-bold">
              {{ job.score.toFixed(1) }}
            </UBadge>
            <UBadge :color="statusColor(job.status)" class="capitalize">
              {{ job.status }}
            </UBadge>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2" @click.stop>
          ...
          />
          <UButton size="xs" variant="ghost" color="error" icon="i-lucide-trash-2" @click="remove(job)" />
          <UButton :to="job.link" target="_blank" size="xs" variant="outline" icon="i-lucide-external-link" @click.stop />
        </div>

        <p v-if="job.notes" class="text-sm text-muted italic">{{ job.notes }}</p>
      </div>
    </div>
  </UContainer>
</template>
